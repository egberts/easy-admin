...
#   DISTDIR - where to distribute the files during install (default /)
#
# System Files impacted:
#   Modified: none
#   Created: none

echo "LigHTTPd Web Server configuration"
echo ""

CHROOT_DIR="${CHROOT_DIR:-}"
BUILDROOT="${BUILDROOT:-build}"

declare sysconfdir  # assigned in distro-os.sh/maintainer-lighttpd.sh
source ./maintainer-lighttpd.sh

if [ "${BUILDROOT:0:1}" != "/" ]; then
  readonly FILE_SETTINGS_FILESPEC="${BUILDROOT}/net-web-lighttpd.sh"
...
lighttpd_conf_filename="${LIGHTTPD_CONFIG_FILESPEC}"
lighttpd_conf_filepath="${LIGHTTPD_CONFIG_FILEPATH}"
flex_chown ${LIGHTTPD_CONFIG_GROUP_NAME}:${LIGHTTPD_CONFIG_USER_NAME} "${lighttpd_conf_filepath}"
flex_chmod ${LIGHTTPD_CONFIG_FILESPEC_PERMS} "${lighttpd_conf_filepath}"
echo "Creating ${BUILDROOT}${CHROOT_DIR}$lighttpd_conf_filepath ..."
cat << LIGHTTPD_CONF_EOF | tee "${BUILDROOT}${CHROOT_DIR}$lighttpd_conf_filepath" >/dev/null
#
# File: ${lighttpd_conf_filename}
# Path: ${LIGHTTPD_CONFIG_FILESPATH}
# Title: LigHTTPd Web Server configuration file
...
server.username             = "www-data"
server.groupname            = "www-data"
server.modules              = "/usr/lib/lighttpd"

include "${LIGHTTPD_ETC_CONF_ENABLED_DIRSPEC}/*.conf"

LIGHTTPD_CONF_EOF

# TODO: Prompt for server-identifier?
# TODO: Prompt for server-name?
lighttpd_options_filename="lighttpd.conf.options"
lighttpd_options_filespec="${extended_sysconfdir}/$lighttpd_options_filename"
flex_chown root:root "${lighttpd_options_filespec}"
flex_chmod 0644 "${lighttpd_options_filespec}"
echo "Creating ${BUILDROOT}${CHROOT_DIR}$lighttpd_options_filespec ..."
cat << LIGHTTPD_CONF_EOF | tee "${BUILDROOT}${CHROOT_DIR}$lighttpd_options_filespec" >/dev/null
#
# File: ${lighttpd_options_filename}
...
# The  LIGHTTPDECLINE  message  is used by DHCP clients to indicate that the
#    lease the server has offered is not valid.  When the server receives  a
#    LIGHTTPDECLINE  for  a  particular  address, it normally abandons that ad‐
#    dress, assuming that some unauthorized system is  using  it.   Unfortu‐
#    nately,  a  malicious  or buggy client can, using LIGHTTPDECLINE messages,
...
#    cations.
#
#    The declines flag tells the DHCP server whether or not to honor LIGHTTPDE‐
#    CLINE  messages.  If it is set to deny or ignore in a particular scope,
#    the DHCP server will not respond to LIGHTTPDECLINE messages.
#
...
#   default is true.
# If update-conflict-detection is true, the hcp-server
...
lighttpd_pool1_filename="lighttpd.conf.pool.10.22.0-vir"
lighttpd_pool1_filespec="${extended_sysconfdir}/$lighttpd_pool1_filename"
flex_chown root:root "${lighttpd_pool1_filespec}"
flex_chmod 0644 "${lighttpd_pool1_filespec}"
echo "Creating ${BUILDROOT}${CHROOT_DIR}$lighttpd_pool1_filespec ..."
cat << LIGHTTPD_CONF_EOF | tee "${BUILDROOT}${CHROOT_DIR}$lighttpd_pool1_filespec" >/dev/null
#
# File: ${lighttpd_pool1_filename}
# Path: ${extended_sysconfdir}
...
lighttpd_pool2_filename="lighttpd.conf.pool.172.17-docker"
lighttpd_pool2_filespec="${extended_sysconfdir}/$lighttpd_pool2_filename"
flex_chown root:root "${lighttpd_pool2_filespec}"
flex_chmod 0644 "${lighttpd_pool2_filespec}"
echo "Creating ${BUILDROOT}${CHROOT_DIR}$lighttpd_pool2_filespec ..."
cat << LIGHTTPD_POOL2_EOF | tee "${BUILDROOT}${CHROOT_DIR}$lighttpd_pool2_filespec" >/dev/null
#
# File: ${lighttpd_pool2_filename}
# Path: ${extended_sysconfdir}
...
lighttpd_pool3_filename="lighttpd.conf.pool.192.168.12-resinet"
lighttpd_pool3_filespec="${extended_sysconfdir}/$lighttpd_pool3_filename"
flex_chown root:root "${lighttpd_pool3_filespec}"
flex_chmod 0644 "${lighttpd_pool3_filespec}"
echo "Creating ${BUILDROOT}${CHROOT_DIR}$lighttpd_pool3_filespec ..."
cat << LIGHTTPD_POOL3_EOF | tee "${BUILDROOT}${CHROOT_DIR}$lighttpd_pool3_filespec" >/dev/null
#
# File: ${lighttpd_pool3_filename}
# Path: ${extended_sysconfdir}
...
# log-facility local7;
  ...
  subnet 192.168.15.0 netmask 255.255.255.0
  {
    pool {
      ...
      # We do static lease via MAC-reservation (see lighttpd.conf.reserved)
      # This pool has static & dynamic IP, cannot use 'deny unknown-clients'
      # deny all clients;
...
lighttpd_pool4_filename="lighttpd.conf.pool.172.32-dmz2"
lighttpd_pool4_filespec="${extended_sysconfdir}/$lighttpd_pool4_filename"
flex_chown root:root "${lighttpd_pool4_filespec}"
flex_chmod 0644 "${lighttpd_pool4_filespec}"
echo "Creating ${BUILDROOT}${CHROOT_DIR}$lighttpd_pool4_filespec ..."
cat << LIGHTTPD_POOL4_EOF | tee "${BUILDROOT}${CHROOT_DIR}$lighttpd_pool4_filespec" >/dev/null
#
# File: ${lighttpd_pool4_filename}
# Path: ${extended_sysconfdir}
...
lighttpd_pool5_filename="lighttpd.conf.pool.192.168.15-wlan"
lighttpd_pool5_filespec="${extended_sysconfdir}/$lighttpd_pool5_filename"
flex_chown root:root "${lighttpd_pool5_filespec}"
flex_chmod 0644 "${lighttpd_pool5_filespec}"
echo "Creating ${BUILDROOT}${CHROOT_DIR}$lighttpd_pool5_filespec ..."
cat << LIGHTTPD_POOL5_EOF | tee "${BUILDROOT}${CHROOT_DIR}$lighttpd_pool5_filespec" >/dev/null
#
# File: ${lighttpd_pool5_filename}
# Path: ${extended_sysconfdir}
...
lighttpd_pools_filename="lighttpd.conf.pools"
lighttpd_pools_filespec="${extended_sysconfdir}/$lighttpd_pools_filename"
flex_chown root:root "${lighttpd_pools_filespec}"
flex_chmod 0644 "${lighttpd_pools_filespec}"
echo "Creating ${BUILDROOT}${CHROOT_DIR}$lighttpd_pools_filespec ..."
cat << LIGHTTPD_POOLS_EOF | tee "${BUILDROOT}${CHROOT_DIR}$lighttpd_pools_filespec" >/dev/null
#
# File: ${lighttpd_pools_filename}
# Path: ${extended_sysconfdir}
# Title: ISC DHCP Server configuration for all DHCP pools
#

include "/etc/dhcp/lighttpd.conf.pool.192.168.12-resinet";
include "/etc/dhcp/lighttpd.conf.pool.192.168.15-wlan";
include "/etc/dhcp/lighttpd.conf.pool.172.32-dmz2";
include "/etc/dhcp/lighttpd.conf.pool.10.22.0-vir";

LIGHTTPD_POOLS_EOF

lighttpd_reserved_filename="lighttpd.conf.reserved"
lighttpd_reserved_filespec="${extended_sysconfdir}/$lighttpd_reserved_filename"
flex_chown root:root "${lighttpd_reserved_filespec}"
flex_chmod 0644 "${lighttpd_reserved_filespec}"
echo "Creating ${BUILDROOT}${CHROOT_DIR}$lighttpd_reserved_filespec ..."
cat << LIGHTTPD_RESERVED_EOF | tee "${BUILDROOT}${CHROOT_DIR}$lighttpd_reserved_filespec" >/dev/null
#
# File: ${lighttpd_reserved_filename}
# Path: ${extended_sysconfdir}
...
LIGHTTPD_RESERVED_EOF

lighttpd_zones_filename="lighttpd.conf.zones"
lighttpd_zones_filespec="${extended_sysconfdir}/$lighttpd_zones_filename"
flex_chown root:root "${lighttpd_zones_filespec}"
flex_chmod 0644 "${lighttpd_zones_filespec}"
echo "Creating ${BUILDROOT}${CHROOT_DIR}$lighttpd_zones_filespec ..."
cat << LIGHTTPD_ZONES_EOF | tee "${BUILDROOT}${CHROOT_DIR}$lighttpd_zones_filespec" >/dev/null
#
# File: ${lighttpd_zones_filename}
# Path: ${extended_sysconfdir}
