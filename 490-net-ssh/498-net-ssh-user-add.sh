...
echo "Adding '$GROUP_NAME' GID to a user's supplementary group"
echo ""
...
read -rp "Add the name of user to '$GROUP_NAME' group [$USER]?: "
if [ -z "$REPLY" ]; then
  SSH_USER="$USER"
else
  SSH_USER="$REPLY"
...
if [ "$USER_HAS_GID" -ge 1 ]; then
  echo "User $SSH_USER already has $GROUP_NAME GID"
  echo ""
  echo "Done."
  exit 0
fi
...
echo "Executing: sudo addgroup $SSH_USER $GROUP_NAME"
sudo usermod -a -G "$GROUP_NAME" "$SSH_USER"
RETSTS=$?
if [ $RETSTS -ne 0 ]; then
  echo "Unable to add '$SSH_USER' user to '$GROUP_NAME' group: Error $RETSTS"
  exit $RETSTS
fi
echo ""
