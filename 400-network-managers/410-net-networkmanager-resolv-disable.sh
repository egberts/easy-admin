#!/bin/bash
# File: 410-net-network-manager-resolv-disabled.sh
# Title: Make NetworkManager stop mucking with /etc/resolv.conf
#
# Prerequisites:
#
# Env varnames
...
echo "Making NetworkManager stop messing with /etc/resolv.conf"
echo ""

# There are two poking mechanism to messing with resolver (/etc/resolv.conf)
#  1. systemd-resolverd
#  2. DHCP-driven DNS update (useful for servers)


FILE_SETTINGS_FILESPEC="${BUILDROOT}${CHROOT_DIR}/file-NetworkManager-resolvers.sh"
